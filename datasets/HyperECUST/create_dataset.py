...
class HyperECUST(Dataset):
    def __init__(self, data_path, filenames, facesize=None, mode='train'):
        """
        Params:
    ...
    def get_image(self, filename, cropped_by_bbox=True):
        # load image array
        if cropped_by_bbox:
            x1, y1, x2, y2 = self.get_bbox(filename)[0]
            image = cv2.imread(os.path.join(
                self.data_path, filename[1:]), cv2.IMREAD_GRAYSCALE)[y1: y2, x1: x2]
        else:
            image = cv2.imread(os.path.join(self.data_path, filename[1:]),
...
if __name__ == '__main__':
    ...
    batch_size = 8
    train_list, valid_list = split_dataset()

    trainset = HyperECUST(DATASET_PATH, train_list,
                          facesize=(128, 128), mode='train')
    validset = HyperECUST(DATASET_PATH, valid_list,
