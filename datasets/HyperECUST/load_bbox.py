...
def load_multi(imgdir, dsize=None):
    ...
    if dsize is None:
        img = cv2.imread(os.path.join(
            imgdir, imgfiles[0]), cv2.IMREAD_GRAYSCALE)
        (h, w) = img.shape
    else:
        (w, h) = dsize
    ...
    for i in range(c):
        img = cv2.imread(os.path.join(
            imgdir, imgfiles[i]), cv2.IMREAD_GRAYSCALE)
...
def draw_bbox(image, score, bbox, landmarks=None):
    """
    Params:
        image:      {ndarray(H, W, 3)}
        score:      {ndarray(n_faces)}
...
if __name__ == "__main__":
    ...
    # Multi
    imgpath = HOME + "/myDataset/ECUST/DATA1/9/Multi/non-obtructive/Multi_4_W1_1"
    ...
    # RGB
    imgpath = HOME + "/myDataset/ECUST/DATA1/9/RGB/non-obtructive/RGB_4_W1_1.JPG"
